...
const sql = readFileSync(
  "supabase/migrations/20260605140000_visitor_locations.sql",
  "utf8"
);
...
try {
  ...
  const count = await client.query(
    "select count(*)::int as n from dev.visitor_locations"
  );
  console.log("Migration OK — dev.visitor_locations rows:", count.rows[0]?.n);
} catch (err) {
